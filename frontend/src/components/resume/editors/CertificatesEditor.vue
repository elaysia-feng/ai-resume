    ...
    <div v-if="isActive">
      ...
      >
        <div class="entry-header">
          <span class="entry-title">证书 {{ index + 1 }}</span>
          <button class="delete-btn" @click="removeEntry(index)">删除</button>
        </div>

        <div class="form-grid">
          <div class="field">
            <label class="field-label">证书名称</label>
            <input
              class="field-input"
              type="text"
              :value="entry.name"
          ...
          <div class="field">
            <label class="field-label">奖项等级</label>
            <select
              class="field-input"
              :value="entry.level"
              @change="updateEntry(index, 'level', $event.target.value)"
            >
          ...
          <div class="field">
            <label class="field-label">获得时间</label>
            <input
              class="field-input"
              type="text"
              :value="entry.date"
              placeholder="如：2023.06"
              @input="updateEntry(index, 'date', $event.target.value)"
            />
          </div>
          <div class="field field-full">
            ...
            <input
              ...
              :value="entry.issuer"
              placeholder="请输入颁发机构"
              @input="updateEntry(index, 'issuer', $event.target.value)"
            />
          </div>
        </div>
